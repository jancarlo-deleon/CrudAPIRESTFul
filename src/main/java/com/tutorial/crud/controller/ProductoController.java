...
public class ProductoController {
    ...
    public ResponseEntity<?> getById(@PathVariable("id") int id) {
        if (!productoService.existsById(id)) {
            return new ResponseEntity<>(new Mensaje("El producto no solicitado no existe"), HttpStatus.NOT_FOUND);
        }
    ...
    @PreAuthorize("hasRole('ADMIN')")
    @DeleteMapping("/{id}")
    public ResponseEntity<Mensaje> delte(@PathVariable("id") int id) {
        if (!productoService.existsById(id)) {
